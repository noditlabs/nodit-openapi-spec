...
Solana의 `programUnsubscribe` 메서드는 `programSubscribe`로 생성된 프로그램 구독을 해제하여 더 이상 프로그램 계정 변경 알림을 받지 않도록 합니다.

> 📘 사용 시 참고사항
>
> - 반드시 **WebSocket** 엔드포인트를 통해 호출해야 하며, HTTP는 지원되지 않습니다.
> - 구독 해제 후에는 해당 subscription ID로 다시 구독할 수 없습니다. 새로운 구독이 필요한 경우 `programSubscribe`를 다시 호출해야 합니다.
