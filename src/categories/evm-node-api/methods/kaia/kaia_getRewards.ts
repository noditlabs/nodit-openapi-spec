...
function oasDocs({
  version,
  protocol,
}: OasParamsWithProtocol): OpenAPIV3.Document {
    ...
    paths: {
      ["/"]: {
        post: {
          ...
          tags: [title],
          description: `블록 넘버를 기준으로 해당 블록의 리워드 분배 결과를 조회합니다. 리워드 수혜자와 각자의 지분이 포함됩니다. 파라미터를 생략하면 최신 블록의 리워드 분배 결과를 반환합니다.`,
          summary: method,
          operationId: slug,
          parameters: [],
          requestBody: {
                ...
                schema: {
                  ...
                  properties: {
                    ...
                    params: {
                      type: "array",
                      items: {
                        oneOf: [Schemas.blockNumberOrTagKaia],
                      },
                      minItems: 0,
                      maxItems: 1,
                      default: [],
                      description: `다음 파라미터들을 타입에 맞게 배열로 입력합니다. 생략 시 최신 블록의 리워드 분배 결과를 반환합니다.
1. \`block number or block tag\` - 조회 대상 블록 식별자로 블록 넘버 또는 블록 태그를 입력할 수 있습니다.
	- 블록 넘버: 정수 또는 16진수 문자열 (ex. "0x1")
