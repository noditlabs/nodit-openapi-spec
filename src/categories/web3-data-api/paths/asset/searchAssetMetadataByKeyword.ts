...
import { whatIsTrc10, throughputLimitInfoMessage } from "../../../../callouts";

const summary = "Search Asset(TRC10) Metadata by Keyword";
const endpoint = "searchAssetMetadataByKeyword";
const isPublic = true;
...
function getDescription(chain: string): string {
  switch (chain) {
    ...
    case "tron":
    default:
      return `Keyword로 검색한 특정 TRC10 리스트 조회합니다.`;
  }
}
  ...
  switch (chain) {
    ...
    default:
      return {
        ...
        successResponse: {
          schema: Domains.Pagination(Domains.Tron.TRC10Meta),
