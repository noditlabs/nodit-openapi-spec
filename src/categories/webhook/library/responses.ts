...
namespace Responses {
  ...
  export const protocol: OpenAPIV3.SchemaObject = {
    type: "string",
    description:
      "해당 Webhook으로 구독하고 있는 체인을 나타내는 필드입니다. (e.g., ETHEREUM, POLYGON, OPTIMISM, ...)",
  };

  export const network: OpenAPIV3.SchemaObject = {
    type: "string",
    description:
      "해당 Webhook으로 구독하고 있는 네트워크를 나타내는 필드입니다. (e.g., MAINNET, SEPOLIA, MUMBAI, ...)",
  };
  ...
  export const condition: OpenAPIV3.SchemaObject = {
    type: "object",
    description:
      "Webhook 구독 조건을 나타내는 필드입니다. 이벤트 타입에 따라 다른 조건을 가질 수 있습니다.",
  };

  export const isInstant: OpenAPIV3.SchemaObject = {
    type: "boolean",
    description: `Instant Webhook 옵션 활성화 여부를 지정하는 필드입니다. 
- true: 이 옵션이 활성화 되어있다면 모니터링 대상 이벤트가 포함된 블록의 확정 여부와 상관 없이 해당 이벤트가 감지되는 즉시 Webhook 메시지를 수신할 수 있습니다. 
- false: 이 옵션이 비활성화 되어있다면 이벤트 트랜잭션이 포함된 블록이 확정된 이후에만 메시지가 발송됩니다.`,
  };
  ...
  export const createdAt: OpenAPIV3.SchemaObject = {
    type: "string",
    description:
      "웹훅 이벤트가 발생하고 로그가 생성된 시점을 나타냅니다. 이 필드는 ISO 8601 형식으로 반환됩니다.",
  };
  ...
  export const result: OpenAPIV3.SchemaObject = {
    type: "boolean",
    description:
      "호출 결과를 나타내는 필드입니다. 성공 시 true, 실패 시 false를 반환합니다.",
  };
