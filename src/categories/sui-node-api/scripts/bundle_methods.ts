...
async function bundleMethod(
  ...
  const inputPath = path.join(methodsDir, methodFile);
...
async function bundleMethods(
  ...
  console.log(`\n=== Bundling SUI HTTP methods ===`);

  if (
    methodToBundle !== "all" &&
    !methodFiles.includes(`${methodToBundle}.yaml`)
  ) {
    console.error(`Method '${methodToBundle}' not found in methods directory`);
    return {
      success: false,
      failedItems: [
        {
          method: methodToBundle,
          error: "Method not found in methods directory",
        },
      ],
    };
  }
  ...
  for (const chunk of fileChunks) {
    const results = await Promise.all(
      chunk.map((methodFile) => bundleMethod(methodFile, methodsDir, outDir))
    );
  ...
  console.log(`\nSUI HTTP methods bundling completed:`);
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.error(
      "  ts-node bundle_methods.ts sui_devInspectTransactionBlock  # 특정 메소드"
    );
    console.error(
  ...
  const outDir = path.join(__dirname, "../../../../reference/sui-node-api");

  if (!(await fs.pathExists(methodsDir))) {
    console.error(`methods directory not found: ${methodsDir}`);
    process.exit(1);
  }

  // methods 폴더에서 모든 .yaml 파일 목록 가져오기
  const methodFiles = await fs.readdir(methodsDir);
  const yamlFiles = methodFiles.filter((file) => file.endsWith(".yaml"));

  if (yamlFiles.length === 0) {
    console.error("No YAML files found in methods directory");
    process.exit(1);
  }
