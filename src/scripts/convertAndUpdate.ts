...
async function main() {
	try {
		...
		if (!apiDefinitionId) {
			throw new Error("❌ Error: API specification not found. Create the API specification first.");
		}
		...
		if (!result) {
			throw new Error(`❌ Error: Failed to update the API specification. (ID: ${apiDefinitionId})`);
		}
