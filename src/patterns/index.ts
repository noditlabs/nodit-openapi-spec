...
export const Patterns = {
	...
	integer: (length?: number) => (length ? new RegExp(`^[-+]?[0-9]{${length}}$`) : new RegExp(`^[-+]?[0-9]+$`)),
	unsignedInteger: (length?: number) => (length ? new RegExp(`^[0-9]{${length}}$`) : new RegExp(`^[0-9]+$`)),
	...
	date: {
		...
		yyyymmddhh: new RegExp(`^[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])-([01][0-9]|2[0-3])$`),
	},
	timestamp: new RegExp(`^[0-9]{10}$`), // UNIX 타임스탬프 (10자리 숫자 문자열)

	// String
	string: {
		decimal: (length?: number) => (length ? new RegExp(`^[0-9]{${length}}$`) : new RegExp(`^[0-9]+$`)),
	...
	aptos: {
		...
		primitiveType: new RegExp(`^(bool|u8|u64|u128|address|signer|vector<.+>|0x[0-9a-zA-Z:_<, >]+)$`),
	},
