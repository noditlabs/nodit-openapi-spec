...
async function main() {
  try {
    ...
    console.log(`🔄 Updating API specification: ${title}`);
    ...
    if (!apiDefinitionId) {
      throw new Error(
        `❌ Error: API specification with title "${title}" not found. Create it first using upload script.`
      );
    }
    ...
    console.log(
      `✅ Successfully updated API specification: ${title} (ID: ${apiDefinitionId})!`
    );
  } catch (error) {
    console.error(
